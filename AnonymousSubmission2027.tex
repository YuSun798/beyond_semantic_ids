...
For a fixed SID model and beam width $B$, target support is the fraction of test users whose held-out target occurs anywhere in the decoded beam. On ML-1M, the canonical seed-42 TIGER-style model has TargetSupport@100 of 63.8\%. This is a per-user quantity closely related to Recall@100; it is not the fraction of catalog items retrieved. Figure~\ref{fig:reachability} preserves the full TIGER popularity breakdown, including the overall mean and the number of users in each quartile.
...
\toprule
Method & Target-in-Output@100 & Q1 rare & Q2 & Q3 & Q4 popular & TargetItemSupport@200 \\
\midrule
...
\caption{TIGER-style SID target support by held-out target popularity on ML-1M, seed 42, beam width 100. Bars report the user-weighted Q1--Q4 hit rates and labels give each quartile's user count and test-set share. The dashed line is the overall TargetSupport@100 of 63.8\% across all 6,040 users.}
