...
respectively. The same training-frequency item-to-quartile mapping is used in
Figure~\ref{fig:crossover} and Table~\ref{tab:crossover}.}
...
\end{figure*}
\textbf{Baselines.}
We compare against SASRec \citep{kang2018sasrec}, TIGER-style SID retrieval \citep{rajput2023tiger}, and reported SID results from TIGER, LETTER, and EAGER where protocols allow contextual comparison. We also include non-learned dense retrieval baselines: random retrieval, last-item kNN, and mean-of-5 kNN.
...
\textbf{CQG-Single} 
is a four-layer, four-head causal Transformer with width 256. It directly generates one normalized query and is trained against the next-item target using full-catalog CE, MSE, or in-batch InfoNCE. \textbf{CQG-AR}
uses a four-layer, six-head causal Transformer with width 384 to autoregres-
% sively generate three residual query updates. Both are
...
Each residual is fed into the next decoding step, while the cumulative query
is updated by adding the residual and then normalizing the
result. 
% All steps refine the same next-item target rather than different SID levels.
In the final configuration, only the final query receives direct full-catalog CE supervision. The multimodal Sports diagnostic instead uses a frozen fusion of collaborative and ImageNet-pretrained ResNet-18 features. The complete details of the implementation are provided in the Appendix~\ref{app:training}.
...
\bottomrule
\end{tabular}
\caption{Cross-dataset primary comparison (mean $\pm$ sample SD; seeds 42--44). ML-1M and Beauty are full-user protocol matched; Sports TIGER uses approximately 5,000 sampled users per seed and is an unpaired diagnostic. Amazon TIGER uses our hierarchical-$k$-means baseline.}
