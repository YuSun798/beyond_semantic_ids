...
\caption{TIGER-style SID target support by held-out target popularity on
ML-1M, seed 42, beam width 100. Bars report the user-weighted Q1--Q4 hit rates
and labels give each quartile's user count and test-set share. The same
training-frequency item-to-quartile mapping is used in
Figure~\ref{fig:crossover} and Table~\ref{tab:crossover}. The dashed line is
the overall TargetSupport@100 of 63.8\% across all 6,040 users.}
...
(Table~\ref{tab:loss}). This auxiliary objective comparison is separate from
joint item-space training; the CE-joint diagnostic is omitted because it
changes both the objective and the item representations.
