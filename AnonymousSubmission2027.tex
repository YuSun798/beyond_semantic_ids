...
\begin{itemize}
    ...
    matched maximum returned-list size.
...
support, maximum-list-size-matched catalog coverage, and popularity-stratified behavior,
...
\caption{Maximum-list-size-matched target support on ML-1M (seed 42), stratified by held-out target popularity. Bars show user-weighted TargetInOutput@100 with at most 100 returned items: TIGER uses beam width 100, while CQG-Single and CQG-AR use dense Top-100 retrieval. The dashed line marks TIGER's overall finite-beam support of 63.8\%. All analyses use the same 6,040 users and popularity-quartile mapping as Figure~\ref{fig:crossover} and Table~\ref{tab:crossover}.}
...
three methods return at most 200 items per user.}
\label{fig:crossover}
\end{figure*}
\textbf{Baselines.}
...
\subsection{Checkpoint- and Budget-Specific Finite-Beam Reachability}

For a fixed model and maximum returned-list size $B$, target support is the fraction of test
...
\bottomrule
\end{tabular}
\caption{ML-1M finite-output target support (seed 42), matched by maximum
returned-list size. TargetInOutput@100 is user weighted. Q1--Q4 use the same
training-frequency item-to-quartile mapping as Figures~\ref{fig:reachability}
and~\ref{fig:crossover}, then compute item-balanced means over 162, 346, 584,
and 781 distinct test-target items, respectively. TargetItemSupport@200 counts
the fraction of all 1,873 distinct test-target items retrieved for at least
one associated test instance. TIGER uses beam widths 100 and 200; CQG uses
dense Top-100 and Top-200 outputs. After mapping, deduplication, and history
filtering, TIGER returns at most rather than necessarily exactly the stated
size. These quantities are empirical finite-output support, not catalog-wide
scoreability.}
...
\subsection{Checkpoint- and Popularity-Dependent Crossover}
...
significant at the 0.05 level. Sports outputs are aligned by the stable test
index and identical target-item sequence. A 10,000-resample seed-42 paired user bootstrap
favors CQG-Single over TIGER by 0.0138 (95\% CI
[0.0110, 0.0165], $p<.0001$) and CQG-AR over TIGER by 0.0139
(95\% CI [0.0111, 0.0167], $p<.0001$); CQG-Single and CQG-AR do not differ
($p=.912$). We therefore interpret crossover as an
...
CQG-Single & One-step & 3.2M & \textbf{0.3104 \(\pm\) 0.0047} & \textbf{0.1822 \(\pm\) 0.0044} & \textbf{0.0597 \(\pm\) 0.0046} & \textbf{0.0363 \(\pm\) 0.0015} \\
...
protocol matched; the final Sports values use the same aligned population of
21,519 test users, split, and history filtering across methods. Amazon TIGER uses our hierarchical-$k$-means
baseline.}
...
output set. With a matched maximum returned-list size of 10 on ML-1M, TIGER, CQG-Single, and
...
\midrule
ML-1M & \textbf{70.1\%} & 95.3\% & 69.7\% & \textbf{97.6\%} & 68.7\% & \textbf{97.6\%} \\
Beauty & 75.2\% & 98.1\% & \textbf{89.5\%} & \textbf{99.9\%} & 87.9\% & 99.8\% \\
Sports & \textbf{92.6\%} & \textbf{100.0\%} & 63.1\% & 96.9\% & 71.8\% & 98.4\% \\
\bottomrule
\end{tabular}
\caption{Maximum-list-size-matched Catalog Coverage: empirical union-of-Top-$K$
catalog coverage for seed 42. All datasets use the same maximum returned-list
...
All three methods use the same frozen SASRec item table
(\texttt{checkpoints/item\_embeddings.pt}), processed split, and evaluation
...
coverage at a matched maximum returned-list size $K$ is
\[
...
At inference, beam search retains at most \(B\) code triples. Exact code
matches are mapped to items; a generated triple without an exact match is
mapped to its nearest catalog code under squared code distance. Multiple
sequences mapping to the same item are collapsed by retaining the highest
score. Previously observed items are then removed. We do not continue beam
expansion or refill after deduplication and history filtering. Consequently,
\(B\) is a maximum returned-list size, not a guarantee of exactly \(B\)
valid, distinct, history-filtered items. At \(B=100\), the seed-42 TIGER-style
evaluator returns on average 83.27 items on ML-1M, 98.06 on Beauty, and 97.64
on Sports (minimum 18, 35, and 72, respectively); the dense CQG evaluators
return exactly 100. This implementation detail is one reason we avoid
generalizing its absolute effect size to all constrained SID decoders.
