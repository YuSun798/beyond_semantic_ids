...
items, respectively. For Sports, the completed full-user CQG-AR output gives
71.8\% and 98.4\% coverage; the corresponding TIGER and CQG-Single cells remain
blank until their queued full-user ranked lists complete, so the Sports row is
not yet a three-method matched comparison.
...
We evaluate a matched 100-candidate retrieve-and-rerank pipeline using legacy
...
CQG-AR MSE + reranker (seed 42) & -- & 0.0486 & 0.0378 \\
...
CQG-AR MSE + reranker (seed 42) & -- & 0.0265 & 0.0194 \\
...
the CQG-AR ML-1M reranking job is pending correction of a checkpoint
sequence-length mismatch. These rows are separate from the CE-trained standalone
models.}
\label{tab:cross}
\end{table}
...
CQG-AR MSE + SASRec & -- & 0.0378 \\
SASRec full-rank & 0.2797 & 0.0390 \\
\bottomrule
\end{tabular}
\caption{Legacy MSE retrieval-quality hierarchy. All methods except SASRec
full-rank use SASRec reranking over top-100 candidates. The CQG-AR ML-1M cell
awaits the corrected reranking run.}
\label{tab:hierarchy}
\end{table}
...
48 (TIGER pending) & -- & -- & 0.3053 & 0.1804 & 0.2940 & 0.1738 \\
\midrule
Mean $\pm$ SD (42--47) &
...
\bottomrule
\end{tabular}
\caption{Interim ML-1M seed expansion. Seeds 42--47 are evaluated on all
6,040 users and summarized in the final row; CQG-Single and CQG-AR seed 48 are
also complete, while TIGER seed 48 is still running and is excluded from that
summary. CQG-Single and CQG-AR use a common dense full-catalog protocol.
TIGER seeds 42--44 use beam 100, whereas seeds 45--47 currently have full-user
beam-200 evaluations; its mean is therefore a mixed-beam diagnostic, not the
final protocol-matched estimate. SD denotes sample standard deviation.
Figure~\ref{fig:crossover} remains a seed-42 matched-budget illustration.}
