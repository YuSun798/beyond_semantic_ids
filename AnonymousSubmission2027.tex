...
\caption{Matched Top-100 target support on ML-1M (seed 42), stratified by held-out target popularity. Bars show user-weighted TargetInOutput@100 under a matched output budget: TIGER uses beam width 100, while CQG-Single and CQG-AR use dense Top-100 retrieval. The dashed line marks TIGER's overall finite-beam support of 63.8\%. All analyses use the same 6,040 users and popularity-quartile mapping as Figure~\ref{fig:crossover} and Table~\ref{tab:crossover}.}
...
intervals in which CQG-Single exceeds TIGER. Vermillion dashed lines mark the
quartile-specific TIGER finite-beam reachability limits at $B=200$, and the
light vermillion regions above them are unreachable for the evaluated
model--beam pair. The Q1 annotation marks the exact measured
...
three methods use the same maximum output budget of 200.}
\label{fig:crossover}
\end{figure*}
\textbf{Baselines.}
...
output set. Under matched Top-10 outputs on ML-1M, TIGER, CQG-Single, and
...
97.6\% of the catalog, compared with 95.3\% for TIGER. The separation is
larger on Beauty at Top-10 (75.2\%, 89.5\%, and 87.9\%), although all methods
...
\midrule
ML-1M & 70.1\% & 95.3\% & 69.7\% & 97.6\% & 68.7\% & 97.6\% \\
Beauty & 75.2\% & 98.1\% & 89.5\% & 99.9\% & 87.9\% & 99.8\% \\
Sports & -- & -- & -- & -- & -- & -- \\
\bottomrule
\end{tabular}
\caption{Empirical union-of-Top-$K$ catalog coverage for seed 42. ML-1M and
Beauty use matched output budgets, user sets, and history filtering across all
three methods; denominators exclude padding and contain 3,416 and 12,101
items, respectively. Sports is included to make the cross-dataset reporting
scope explicit, but matched coverage cannot be reconstructed from the saved
outputs: its TIGER run retained aggregate metrics for sampled users without
ranked item lists, and its CQG-Single run retained aggregate metrics only.
This output-union metric is distinct from per-user TargetSupport and
catalog-wide scoreability.}
