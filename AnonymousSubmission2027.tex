...
\textbf{CQG-Single} generates one normalized query, whereas \textbf{CQG-AR}
autoregressively refines a cumulative query using residual updates. Both are
trained against the same next-item target in the frozen item space. Complete
architectures, objectives, sample construction, optimization settings, and
checkpoint selection are provided in Appendix~\ref{app:training}.
...
Additional negative diagnostics on pretrained LLM backbones and SID
tokenization stability are reported in Appendices~\ref{app:llm}
and~\ref{app:tokenization}, respectively.
...
reached R@10 = 0.2118 on ML-1M but still trailed the scratch Transformer.
...
text-aware Qwen hybrid still trails the scratch query Transformer.}
