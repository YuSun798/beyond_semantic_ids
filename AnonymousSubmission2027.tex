...
reported in Appendix~\ref{app:crossover-values}. Table~\ref{tab:primary-comparison}
...
per-user outputs are not aligned across model families. We therefore interpret crossover as an
...
\midrule
SASRec & Full-rank baseline & \shortstack{dataset-\\dependent} & $0.2797$ & $0.1535$ & $0.0558$ & $\mathbf{0.0390}$ \\
TIGER & Discrete AR & $5.78\mathrm{M}$ & $0.2996{\pm}0.0072$ & $0.1739{\pm}0.0046$ & $0.0474{\pm}0.0044$ & $0.0238{\pm}0.0024$ \\
CQG-Single & Single-step & $3.2\mathrm{M}$ & $\mathbf{0.3104{\pm}0.0047}$ & $\mathbf{0.1822{\pm}0.0044}$ & $\mathbf{0.0597{\pm}0.0046}$ & $\mathbf{0.0363{\pm}0.0015}$ \\
CQG-AR & Autoregressive ($L=3$) & $5.62\mathrm{M}$ & $0.2976{\pm}0.0032$ & $0.1735{\pm}0.0016$ & $0.0571{\pm}0.0043$ & $0.0355{\pm}0.0004$ \\
\bottomrule
\end{tabular}
...
Appendix~\ref{app:seed-stability}. SASRec parameter counts are dataset dependent because its
item embedding table scales with catalog size. ML-1M and Beauty are full-user
protocol matched; the final Sports values use all 21,519 test users but remain
unpaired across methods. Amazon TIGER uses our hierarchical-$k$-means
baseline.}
...
coverage. Sports provides a useful counterexample to interpreting coverage as
accuracy: TIGER covers 92.6\% of the catalog in the union of seed-42 Top-10
lists, substantially more than CQG-Single (63.1\%) or CQG-AR (71.8\%), yet
has lower R@10. Broad output diversity and target ranking quality are distinct.
...
\midrule
ML-1M & $\mathbf{70.1\%}$ & $95.3\%$ & $69.7\%$ & $\mathbf{97.6\%}$ & $68.7\%$ & $\mathbf{97.6\%}$ \\
Beauty & $75.2\%$ & $98.1\%$ & $\mathbf{89.5\%}$ & $\mathbf{99.9\%}$ & $87.9\%$ & $99.8\%$ \\
Sports & $\mathbf{92.6\%}$ & $\mathbf{100.0\%}$ & $63.1\%$ & $96.9\%$ & $71.8\%$ & $98.4\%$ \\
\bottomrule
\end{tabular}
\caption{Matched Catalog Coverage: empirical union-of-Top-$K$ catalog coverage
for seed 42. All datasets use matched output budgets, full test-user sets, and
history filtering across the three methods; denominators exclude padding and
...
\midrule
$L=1$ & $0.2924$ & $\mathbf{0.1719}$ & $0.6550$ \\
$L=2$ & $0.2909$ & $0.1688$ & $0.6525$ \\
$L=3$ & $0.2914$ & $0.1667$ & $0.6619$ \\
$L=4$ & $\mathbf{0.2944}$ & $0.1717$ & $\mathbf{0.6697}$ \\
\bottomrule
\end{tabular}
...
accuracy, and the single-step boundary remains competitive with the three-step
control. The resulting latency, memory, index-update cost, and cold-item
...
Sports & 42 & 0.0212 & 0.0107 & 0.0433 & 0.0857 & 9.06\% \\
Sports & 43 & 0.0242 & 0.0126 & 0.0507 & 0.1010 & 10.72\% \\
Sports & 44 & 0.0260 & 0.0135 & 0.0541 & 0.1058 & 11.16\% \\
\bottomrule
\end{tabular}
\caption{Three-seed TIGER-style SID diagnostics on Beauty and Sports. Each
row uses the same leave-one-out evaluator, beam width 100, and the full test
user set (22,363 Beauty users or 21,519 Sports users). The low
...
under the final common protocol. TIGER uses beam width 100 for R@10; using
beam width 200 leaves R@10 unchanged and raises mean TargetSupport from
$0.6352\pm0.0034$ to $0.7336\pm0.0025$. Across seven seeds, the 95\% $t$
intervals for R@10 are $[0.2930,0.3063]$ for TIGER,
$[0.3060,0.3147]$ for CQG-Single, and $[0.2947,0.3006]$ for CQG-AR.
These intervals quantify training-seed variation; they are distinct from the
paired seed-42 user bootstrap in Appendix~\ref{app:crossover-values}.
...
44 & 0.2841 & 0.1643 & 0.3144 & 0.1831 & 0.2945 & 0.1711 \\
...
\bottomrule
\end{tabular}
\caption{Final seven-seed ML-1M results. Each row evaluates all 6,040 test
users with the same history filtering. N@10 denotes NDCG@10.}
...
use the legacy MSE retriever; CQG-AR uses the final available seed-42
...
We do not report the earlier 200-trial matched-target table because its
within-seed item-resampling standard deviation is not an estimate of
training-seed uncertainty and its legacy target mapping is not traceable to
the final checkpoints. The final paper instead reports direct item-balanced
support in Table~\ref{tab:target-support-comparison}, matched union coverage in
Table~\ref{tab:matched-coverage}, and explicit training-seed variation in
Table~\ref{tab:ml1m-seven-seed}.
