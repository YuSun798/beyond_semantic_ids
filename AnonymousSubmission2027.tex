...
output dimensionality when encoded items are inserted. CQG-Single has the
highest mean shallow accuracy in the current three-seed comparison, while
CQG-AR shows that autoregressive modeling can be retained with continuous
outputs. However, additional continuous refinement does not monotonically
improve shallow accuracy, and the single-step boundary remains competitive
with the three-step control. The resulting latency, memory, index-update cost,
and cold-item accuracy have not been measured at production scale.
