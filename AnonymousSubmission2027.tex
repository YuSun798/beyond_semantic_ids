...
\paragraph{Primary cross-dataset comparison.}
...
full-user protocol matched; its paired seed-42 comparison favors CQG-Single at
$K=10,50,90$ (all $p<.001$). Sports TIGER still uses sampled users and is
shown only as an unpaired diagnostic.
...
\paragraph{Cross-dataset pipeline results.}
We also evaluate a legacy MSE-trained CQG-Single checkpoint as a candidate
generator followed by SASRec reranking. Table~\ref{tab:cross} shows that this
pipeline achieves 97.7--102\% of SASRec Recall@10 across the three datasets.
These candidate-generation diagnostics are separate from the CE-trained
standalone results in Table~\ref{tab:primary-comparison}.
