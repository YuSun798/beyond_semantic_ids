...
comparison, while Table~\ref{tab:crossover} and Figure~\ref{fig:crossover}
examine retrieval depth and target popularity on ML-1M. Under the same
6,040-user protocol, CQG-Single is strongest at shallow retrieval, whereas
CQG-AR approximately matches TIGER-style accuracy with lower seed variance.
...
protocol-matched seed-42 comparison favors CQG-Single at $K=10,50,90$ (all
$p<.001$). Sports remains an unpaired diagnostic because its TIGER runs use
sampled users. We therefore interpret crossover as an operating-point-dependent
empirical pattern rather than a universal dominance claim.
...
SASRec Recall@10 (Table~\ref{tab:cross}). To determine whether this performance
merely results from averaging recent item embeddings,
Table~\ref{tab:hierarchy} compares the learned retriever with random,
last-item kNN, and mean-of-five kNN baselines under the same SASRec reranker.
Random retrieval fails, heuristic kNN recovers part of the signal, and learned
query generation closes most of the remaining gap to full-rank SASRec. These
candidate-generation diagnostics are separate from the CE-trained standalone
results in Table~\ref{tab:primary-comparison}.
